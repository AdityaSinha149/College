...
typedef struct token{
    char tokenName [50];   // logical token type (keyword, id, Identifier, stringLit, operator, symbol, ...)
    char tokenValue [50];  // lexeme to print in <TokenName,row,col>
    char tokenType [50];
...
//global variables
// For this midsem, treat only JS/jQuery specific keywords as language keywords.
...
static token getNextToken(FILE *src, int *row, int *col){
    ...
    while (ch != EOF) {
        ...
        // Identifiers start with an alphabetic character or underscore.
        // '$' is treated as an operator in this midsem.
        if (isalpha(ch) || ch == '_') {
...
static token isKeyword(int ch, FILE *src, int *row, int *col) {
    ...
    for (int k = 0; k < (int)(sizeof(keywords)/sizeof(keywords[0])); k++) {
        if (strcmp(word, keywords[k]) == 0) {
            // Store the actual lexeme in tokenValue so that
            // <TokenName,row,col> prints the keyword itself.
...
static token isStringLiteral(int quote, FILE *src, int *row, int *col) {
    token curr;
    memset(&curr, 0, sizeof(curr));
    ...
    // Rule 3 of the question: strings starting with '#' and
    // without spaces are treated as identifiers (e.g. "#submit").
    if (buf[0] == '#' && strchr(buf, ' ') == NULL) {
        strcpy(curr.tokenName, "Identifier");
        strncpy(curr.tokenValue, buf, sizeof(curr.tokenValue) - 1);
        curr.tokenValue[sizeof(curr.tokenValue) - 1] = '\0';
    } else {
